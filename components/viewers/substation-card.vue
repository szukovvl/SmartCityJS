<template>
  ...
  >
    <v-card-title>
      ...
      &laquo;{{ station !== undefined ? station.identy : '' }}&raquo;
    </v-card-title>
    <v-divider
      class="info"
      style="opacity: 0.35"
    ...
    <v-card-text class="d-flex flex-wrap justify-space-around">
      <SubnetControl
        ...
        class="ma-1"
        :line="line"
        :tariff-data="$store.state.tariffs !== undefined ? $store.state.tariffs.tariff.tk_low : 0"
      />
    </v-card-text>
  </v-card>
</template>
...
<script>
import SubnetControl from '~/components/viewers/subnet-card.vue'
